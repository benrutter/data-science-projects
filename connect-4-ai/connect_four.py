...
def maverick_evaluate_board(board):
...
def count_twos_evaluate_board(board):
    if has_won(board, 'X'):
        return float('Inf')
    elif has_won(board, 'O'):
        return -float('Inf')
        ...
        for col in range(len(board)):
            for row in range(len(board[0])):
                ...
                if board[row] == 0:
                    rowr = [1]
                if board[col] == len(board):
                    colr = [-1]
                if board[row] == len(board[0]):
                    rowr = [-1]
                if board[col][row] == 'X':
                    for c in colr:
                        for r in rowr:
                ...
                if board[col][row] == 'X':
                    for c in colr:
                        for r in rowr:
                            rr = row + r
                            cc = col + c
                            if (rr < len(board[0]) and rr > 0) and (cc < len(board) and cc > 0):
                                if board[cc][rr] == 'X' and (c+r != 0):
                                    x_touch_score += streakfactor
                                    nr = row + r + r
                                    nc = col + c + c
                                    if (nr < len(board[0]) and nr > 0) and (nc < len(board) and nc > 0):
                                        if board[nc][nr] == 'X':
                                            x_touch_score += streakfactor ** 2
                                            nnr = nr + r
                                            nnc = nc + c
                                            if (nnr < len(board[0]) and nnr > 0) and (nnc < len(board) and nnc > 0):
                                                if board[nnc][nnr] == 'O':
                                                    x_touch_score -= ((streakfactor ** 2) - hatetouching)
                                                else:
                                                    x_touch_score += opportunity ** 3
                                        if board[nc][nr] == 'O':
                                            x_touch_score -= (hatetouching + streakfactor)
                                        else:
                                            x_touch_score += opportunity ** 2
                                if board[cc][rr] == 'O':
                                    x_touch_score -= hatetouching
                                else:
                                    x_touch_score += opportunity
                if board[col][row] == 'O':
                    for c in colr:
                        for r in rowr:
                            rr = row + r
                            cc = col + c
                            if (rr < len(board[0]) and rr > 0) and (cc < len(board) and cc > 0):
                                if board[cc][rr] == 'O' and (c+r != 0):
                                    o_touch_score += streakfactor
                                    nr = row + r + r
                                    nc = col + c + c
                                    if (nr < len(board[0]) and nr > 0) and (nc < len(board) and nc > 0):
                                        if board[nc][nr] == 'O':
                                            o_touch_score += streakfactor ** 2
                                            nnr = nr + r
                                            nnc = nc + c
                                            if (nnr < len(board[0]) and nnr > 0) and (nnc < len(board) and nnc > 0):
                                                if board[nnc][nnr] == 'X':
                                                    o_touch_score -= ((streakfactor ** 2) - hatetouching)
                                                else:
                                                    o_touch_score += opportunity ** 3
                                        if board[nc][nr] == 'X':
                                            o_touch_score -= ((streakfactor ** 2) + hatetouching)
                                        else:
                                            o_touch_score += opportunity ** 2
                                if board[cc][rr] == 'X':
                                    o_touch_score -= hatetouching
                                else:
                                    o_touch_score += opportunity
        return x_touch_score - o_touch_score        

def two_ai_game(r1, r2):
    my_board = make_board()
    while not game_is_over(my_board):
      #The "X" player finds their best move.
      result = minimax(my_board, True, 4, -float("Inf"), float("Inf"), r1[0])
      os.system('clear')
      ...
      if not game_is_over(my_board):
        ...
        result = minimax(my_board, False, 4, -float("Inf"), float("Inf"), r2[0])
        os.system('clear')
